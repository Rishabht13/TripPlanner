...
function HotelsAds() {
  ...
  useEffect(() => {
    ...
    adsAPI.getAll('hotels').then(res => setItems(res.data)).catch(() => {});
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Hotel Ads</h1>
          <p className="mt-1 text-gray-600">Browse stays and resorts</p>
        </div>
      </div>

      <div className="bg-white rounded-lg shadow p-4 mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex-1">
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search hotels..." className="w-full border rounded px-3 py-2" />
        </div>
